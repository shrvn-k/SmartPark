...
def getPlateRegion():
	...
	#ret,thresh_image = cv2.threshold(sub_morp_image,52,255,cv2.THRESH_BINARY)
	...
	for c in contours:
		...
		ar=float(w)/float(h)
		#cv2.imshow("c--"+str(ar),lol)
		#cv2.waitKey()
		# cv2.waitKey()
		#if  (w>175) and (h>60) and (w<530) and (h<120):
		#	screenCnt=c
		#if  (w>175) and (h>80) and (w<530) and (h<185):
		if (ar>2.85) and (ar<5.5):
                        screenCnt=c
                        break
